...
class SimpleAnalytics {
    ...
    ensureCSVHeaders() {
        ...
        if (!fs.existsSync(this.sessionsFile)) {
            this.appendCSV(this.sessionsFile, 'session_id,user_id,repo_url,repo_name,branch,head_commit,started_at,ended_at,turn_count,total_cost_usd,interrupted_turns');
        }
        ...
        if (!fs.existsSync(this.commitsFile)) {
            this.appendCSV(this.commitsFile, 'commit_sha,session_id,user_id,repo_name,branch,commit_message,author_email,committed_at,files_changed,insertions,deletions,total_loc_changed');
        }
        ...
        if (!fs.existsSync(this.costsFile)) {
            this.appendCSV(this.costsFile, 'session_id,user_id,turn_number,message_id,input_tokens,output_tokens,total_tokens,input_cost_usd,output_cost_usd,total_cost_usd,timestamp');
        }
        ...
        if (!fs.existsSync(this.compactionsFile)) {
            this.appendCSV(this.compactionsFile, 'session_id,user_id,turn_number,timestamp,tokens_before,tokens_after,reduction_tokens,reduction_percent,compaction_type,trigger_reason');
        }
    }
    ...
    async handleCompaction(sessionId, eventData) {
        ...
        const reductionPercent = tokensBefore > 0 ?
            ((reductionTokens) / tokensBefore * 100).toFixed(2) : 0;
    ...
    updateSessionRecord(sessionId, turnCount, totalCost, wasInterrupted, endTime) {
        try {
            ...
            // Get header
            const header = sessions.length > 0 ? sessions[0] : 'session_id,user_id,repo_url,repo_name,branch,head_commit,started_at,ended_at,turn_count,total_cost_usd,interrupted_turns';
